...
class BaselineLGBM:
    ...
    def split_data_full_validation(self, train_df: pl.DataFrame, test_df: pl.DataFrame, horizon: int) -> Tuple[pl.DataFrame, pl.DataFrame]:
        """
    ...
    def train_horizon_time_validation(self, train_df: pl.DataFrame, horizon: int, feature_cols: List[str]) -> ValidationResults:
    ...
    def train_horizon_full_validation(self, train_df: pl.DataFrame, test_df: pl.DataFrame, horizon: int, feature_cols: List[str]) -> ValidationResults:
        ...
        # For test data, we can't evaluate without true targets
        # Just return model info without metrics
        return ValidationResults(
            train_metrics=None,  # Not needed for full validation
            valid_metrics=None,  # Not needed for full validation
            test_metrics=None,   # No true targets available for test
            model=model,
            feature_names=feature_cols,
            training_time_seconds=training_time
        )
    
    def train_all_horizons(self, train_df: pl.DataFrame, test_df: Optional[pl.DataFrame] = None) -> Dict[int, Dict[str, ValidationResults]]:
        """
        ...
        for horizon in self.horizons:
            ...
            if test_df is not None:
                ...
                # Print full validation info (no metrics available for test)
                self.logger.info(f"Full Validation - Horizon {horizon}: Model trained on full data, {len(test_df.filter(pl.col('horizon') == horizon)):,} test samples ready for prediction")
    ...
    def generate_submission(self, train_df: pl.DataFrame, test_df: pl.DataFrame, results: Dict[int, Dict[str, ValidationResults]], filename: Optional[str] = None) -> Path:
    ...
    def print_summary(self, results: Dict[int, Dict[str, ValidationResults]]) -> None:
        ...
        for horizon in self.horizons:
            ...
            if 'full_validation' in horizon_results:
                ...
                if full_res.test_metrics:
                    print(f"{horizon:<8} {'Full Valid':<12} {full_res.test_metrics.weighted_rmse:<15.6f} "
                          f"{full_res.test_metrics.pearson:<12.6f} {full_res.test_metrics.rmse:<12.6f} "
        ...
        for horizon in self.horizons:
            if horizon in results:
                if 'time_validation' in results[horizon]:
                    time_wrmses.append(results[horizon]['time_validation'].valid_metrics.weighted_rmse)
                if 'full_validation' in results[horizon] and results[horizon]['full_validation'].test_metrics:
                    full_wrmses.append(results[horizon]['full_validation'].test_metrics.weighted_rmse)
        
        if time_wrmses:
            print(f"Average Time Validation Weighted RMSE: {np.mean(time_wrmses):.6f}")
        if full_wrmses:
            print(f"Average Full Validation Weighted RMSE: {np.mean(full_wrmses):.6f}")
