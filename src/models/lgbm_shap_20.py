...
Based on lgbm_shap_10.py pattern with top_20 data.
...
class LGBM_SHAP_20:
    ...
    - Top 20 SHAP features from existing SHAP analysis
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        """Initialize LGBM SHAP 20 model."""
        self.config = config or {}
        self.logger = logging.getLogger(__name__)

        # Paths
        project_root = Path(__file__).parent.parent.parent
        self.processed_dir = project_root / 'data/processed/top_20'
        self.models_dir = project_root / 'results/models/lgbm_shap_20'
        self.predictions_dir = project_root / 'results/predictions/lgbm_shap_20'
    ...
    def prepare_engineered_data(self) -> Dict[int, Tuple[pl.DataFrame, pl.DataFrame]]:
        ...
        if missing:
            raise FileNotFoundError(
                ...
                f"SOLUTION: Run feature engineering first with n_features=20:\n"
                f"  from features.shap_features import SHAPFeatureEngineer\n"
                f"  engineer = SHAPFeatureEngineer()\n"
                f"  engineer.run_full_pipeline(train, test, n_features=20)\n"
                f"{'=' * 60}"
            )
    ...
    def _save_metrics(self, horizon: int, train_metrics: MetricResults,
        ...
        # Save JSON
        json_path = self.metrics_dir / f'metrics_h{horizon}_lgbm_shap_20.json'
    ...
    def train_horizon(self, horizon: int, train_df: pl.DataFrame, test_df: pl.DataFrame) -> ValidationResults:
        ...
        # Save predictions
        pred_path = self.predictions_dir / f'lgbm_shap_20_h{horizon}_predictions.npz'
        ...
        # Print results
        print(f"\n  {'=' * 50}")
        print(f"  HORIZON {horizon} RESULTS (TOP 20 SHAP)")
        print(f"  {'=' * 50}")
    ...
    def train_all_horizons(self) -> Dict[int, ValidationResults]:
        ...
        self.logger.info("Starting LGBM SHAP-20 training...")
    ...
    def generate_final_submission(self, results: Dict[int, ValidationResults]) -> Path:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        submission_path = self.predictions_dir / f'lgbm_shap_20_submission_{timestamp}.csv'
        submission_df.write_csv(submission_path)
    ...
    def _print_summary(self, results: Dict[int, ValidationResults], total_time: float) -> None:
        ...
        print(f"\n{'=' * 80}")
        print("LGBM SHAP-20 - FINAL SUMMARY")
        print(f"{'=' * 80}")
