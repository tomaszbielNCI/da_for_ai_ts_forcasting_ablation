#!/usr/bin/env python3
"""
CatBoost Model with Top 10 SHAP Features
...
class CatBoostModel:
    """
    CatBoost model using top 10 SHAP features per horizon.
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        """Initialize CatBoost SHAP 10 model with configuration."""
        self.config = config or {}
        self.logger = logging.getLogger(__name__)
        
        # Paths
        self.processed_dir = Path('data/processed/top_10')
        self.models_dir = Path('results/models/catboost_shap_10')
        self.predictions_dir = Path('results/predictions/catboost_shap_10')
        ...
        # Horizon-specific parameters for CatBoost (adapted from notebook)
        self.horizon_params = {
            1: {'depth': 10, 'learning_rate': 0.028, 'min_data_in_leaf': 77, 'l2_leaf_reg': 3.89, 'subsample': 0.85},
            3: {'depth': 10, 'learning_rate': 0.05, 'min_data_in_leaf': 20, 'l2_leaf_reg': 0.1, 'subsample': 0.85},
            10: {'depth': 10, 'learning_rate': 0.05, 'min_data_in_leaf': 20, 'l2_leaf_reg': 0.1, 'subsample': 0.85},
            25: {'depth': 16, 'learning_rate': 0.021, 'min_data_in_leaf': 250, 'l2_leaf_reg': 20.0, 'subsample': 0.85}
        }
        
        # Seeds
        self.seeds = self.config.get('seeds', [42])
    
    def get_catboost_params(self, horizon: int, seed: int = 42) -> Dict[str, Any]:
        """Get CatBoost parameters for specific horizon."""
        ...
        return {
            ...
            'iterations': 1000,
            'min_data_in_leaf': h_params['min_data_in_leaf'],
            'subsample': h_params['subsample'],
            'colsample_bylevel': 0.75,
            'l2_leaf_reg': h_params['l2_leaf_reg'],
            'random_seed': seed,
            'verbose': False
        }
    ...
    def train_model(self, X_train: np.ndarray, y_train: np.ndarray, w_train: np.ndarray, 
                    horizon: int, seed: int = 42) -> cb.CatBoostRegressor:
        """Train CatBoost model."""
        params = self.get_catboost_params(horizon, seed)
    ...
    def train_horizon_time_validation(self, horizon: int, train_df: pl.DataFrame) -> ValidationResults:
        ...
        self.logger.info(f"Training CatBoost SHAP-10 for H={horizon} with time validation...")
        ...
        # Save predictions
        predictions_path = self.predictions_dir / f'catboost_shap_10_h{horizon}_predictions.npz'
    ...
    def train_horizon_full_validation(self, horizon: int, train_df: pl.DataFrame, 
        ...
        self.logger.info(f"Training CatBoost SHAP-10 for H={horizon} with full validation...")
        ...
        # Save model
        model_path = self.models_dir / f'catboost_shap_10_h{horizon}_full.pkl'
        import pickle
        with open(model_path, 'wb') as f:
            pickle.dump(model, f)
        
        # Save predictions and create submission
        predictions_path = self.predictions_dir / f'catboost_shap_10_h{horizon}_full_predictions.npz'
        np.savez(predictions_path, 
                train_pred=y_train_pred, 
    ...
    def create_submission_file(self, horizon: int, test_df: pl.DataFrame, predictions: np.ndarray) -> None:
        ...
        submission_path = self.predictions_dir / f'catboost_shap_10_h{horizon}_submission_{timestamp}.csv'
    ...
    def train_all_horizons(self) -> Dict[int, Dict[str, ValidationResults]]:
        ...
        self.logger.info("Starting CatBoost SHAP-10 training for all horizons...")
    ...
    def print_summary(self, results: Dict[int, Dict[str, ValidationResults]], 
        ...
        print(f"\n{'='*80}")
        print("CATBOOST SHAP-10 TRAINING SUMMARY")
        print(f"{'='*80}")
