...
class CatBoostModel:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        """Initialize CatBoost SHAP 10 model."""
        ...
        # Horizon-specific parameters for CatBoost (with early stopping)
        self.horizon_params = {
    ...
    def get_catboost_params(self, horizon: int) -> Dict[str, Any]:
    ...
    def train_horizon(self, horizon: int, train_df: pl.DataFrame, test_df: pl.DataFrame) -> ValidationResults:
        ...
        # Train with early stopping (CatBoost works well with early stopping)
        params = self.get_catboost_params(horizon)
        ...
        final_model.set_params(iterations=best_iter)
        ...
        # Save predictions
        pred_path = self.predictions_dir / f'catboost_shap_10_h{horizon}_predictions.npz'
        ...
        # Print results
        print(f"\n  {'=' * 50}")
        print(f"  HORIZON {horizon} RESULTS (CatBoost SHAP-10)")
        print(f"  {'=' * 50}")
    ...
    def train_all_horizons(self) -> Dict[int, ValidationResults]:
        ...
        self.logger.info("Starting CatBoost SHAP-10 training...")
    ...
    def generate_final_submission(self, results: Dict[int, ValidationResults]) -> Path:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        submission_path = self.predictions_dir / f'catboost_shap_10_submission_{timestamp}.csv'
        submission_df.write_csv(submission_path)
    ...
    def _print_summary(self, results: Dict[int, ValidationResults], total_time: float) -> None:
        ...
        print(f"\n{'=' * 80}")
        print("CatBoost SHAP-10 - FINAL SUMMARY")
        print(f"{'=' * 80}")
