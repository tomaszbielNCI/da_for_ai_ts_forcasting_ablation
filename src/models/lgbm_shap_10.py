#!/usr/bin/env python3
"""
LGBM Model with Top 10 SHAP Features

LightGBM model using top 10 SHAP features per horizon with horizon-specific parameters.
...
class LGBM_SHAP_10:
    """
    LightGBM model using top 10 SHAP features per horizon.
    ...
    def prepare_engineered_data(self) -> Dict[int, Tuple[pl.DataFrame, pl.DataFrame]]:
        ...
        if not self.check_shap_results():
            raise FileNotFoundError(
                ...
                f"  from models.shap_analyzer import SHAPAnalyzer\n"
                f"  analyzer = SHAPAnalyzer()\n"
                f"  analyzer.run_analysis()\n"
                f"{'=' * 60}"
            )
        ...
        if missing:
            raise FileNotFoundError(
                ...
                f"SOLUTION: Run feature engineering first:\n"
                f"  from features.shap_features import SHAPFeatureEngineer\n"
                f"  import polars as pl\n"
                f"  train = pl.read_parquet('data/cleaned/train_clean.parquet')\n"
                f"  test = pl.read_parquet('data/cleaned/test_clean.parquet')\n"
                f"  engineer = SHAPFeatureEngineer()\n"
                f"  engineer.run_full_pipeline(train, test, n_features=10)\n"
                f"{'=' * 60}"
            )
    ...
    def get_lgbm_params(self, horizon: int) -> Dict[str, Any]:
        """Get LightGBM parameters for specific horizon with early stopping."""
    ...
    def _save_metrics(self, horizon: int, train_metrics: MetricResults,
        ...
        # Save JSON
        json_path = self.metrics_dir / f'metrics_h{horizon}_lgbm_shap_10.json'
    ...
    def train_horizon(self, horizon: int, train_df: pl.DataFrame, test_df: pl.DataFrame) -> ValidationResults:
        ...
        Uses time split: train (ts <= 3000), validation (3001-3600)
        """
        ...
        # Evaluate on training data (using TimeSeriesMetrics)
        ...
        # Save predictions
        pred_path = self.predictions_dir / f'lgbm_shap_10_h{horizon}_predictions.npz'
    ...
    def train_all_horizons(self) -> Dict[int, ValidationResults]:
        ...
        self.logger.info("Starting LGBM SHAP-10 training...")
    ...
    def generate_final_submission(self, results: Dict[int, ValidationResults]) -> Path:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        submission_path = self.predictions_dir / f'lgbm_shap_10_submission_{timestamp}.csv'
        submission_df.write_csv(submission_path)
    ...
    def _print_summary(self, results: Dict[int, ValidationResults], total_time: float) -> None:
        ...
        print(f"\n{'=' * 80}")
        print("LGBM SHAP-10 - FINAL SUMMARY")
        print(f"{'=' * 80}")
