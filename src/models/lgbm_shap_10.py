...
class LGBM_SHAP_10:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        """Initialize LGBM SHAP 10 model with configuration."""
        self.config = config or {}
        self.logger = logging.getLogger(__name__)
        
        # Paths
        self.processed_dir = Path('data/processed/top_10')
        self.models_dir = Path('results/models/lgbm_shap_10')
        self.predictions_dir = Path('results/predictions/lgbm_shap_10')
        ...
        for col in df.columns:
            if col not in exclude_cols:
                ...
                if dtype in [pl.Float32, pl.Float64, pl.Int32, pl.Int64, pl.UInt32, pl.UInt64]:
                    ...
                    self.logger.warning(f"Excluding non-numeric column: {col} (dtype: {dtype})")
