...
class LGBM_All_Plus_SHAP:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        """Initialize LGBM All Plus SHAP model with configuration."""
        self.config = config or {}
        self.logger = logging.getLogger(__name__)
        
        # Paths
        self.processed_dir = Path('data/processed/all_plus_10')
        self.models_dir = Path('results/models/lgbm_all_plus_shap')
        self.predictions_dir = Path('results/predictions/lgbm_all_plus_shap')
    ...
    def train_horizon_time_validation(self, horizon: int, train_df: pl.DataFrame) -> ValidationResults:
        ...
        self.logger.info(f"Training LGBM All+SHAP for H={horizon} with time validation...")
        ...
        # Save predictions
        predictions_path = self.predictions_dir / f'lgbm_all_plus_shap_h{horizon}_predictions.npz'
    ...
    def train_horizon_full_validation(self, horizon: int, train_df: pl.DataFrame, 
        ...
        self.logger.info(f"Training LGBM All+SHAP for H={horizon} with full validation...")
        ...
        # Save model
        model_path = self.models_dir / f'lgbm_all_plus_shap_h{horizon}_full.pkl'
        import pickle
        with open(model_path, 'wb') as f:
            pickle.dump(model, f)
        
        # Save predictions and create submission
        predictions_path = self.predictions_dir / f'lgbm_all_plus_shap_h{horizon}_full_predictions.npz'
        np.savez(predictions_path, 
                train_pred=y_train_pred, 
    ...
    def train_all_horizons(self) -> Dict[int, Dict[str, ValidationResults]]:
        ...
        self.logger.info("Starting LGBM All+SHAP training for all horizons...")
    ...
    def print_summary(self, results: Dict[int, Dict[str, ValidationResults]]) -> None:
        ...
        print(f"\n{'='*80}")
        print("LGBM ALL+SHAP TRAINING SUMMARY")
        print(f"{'='*80}")
